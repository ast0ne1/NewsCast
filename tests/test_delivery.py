...
def test_delivery_scheduled_when_unpublished(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("app.services.delivery.dated_briefing_path", lambda day, suffix="epub": tmp_path / f"news-{day}.epub")
    monkeypatch.setattr("app.services.briefing.dated_briefing_path", lambda day, suffix="epub": tmp_path / f"news-{day}.epub")
